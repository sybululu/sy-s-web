...
系统采用**单项目双模式**设计，统一域名下根据登录状态展示不同界面：

```
用户访问 https://xxx.dev.coze.site
        │
        ▼
   ┌────────────┐
   │ 已有 Token? │
...
| 模式 | 触发条件 | 展示内容 | 核心组件 |
|------|---------|---------|---------|
| **营销站模式** | 未登录（无有效 JWT） | Hero 营销页面 + 登录/注册弹窗 | `Navbar` + `Landing` + `Footer` + `Login/Register`(overlay) |
| **产品模式** | 已登录（localStorage 有 token） | B 端合规审查工作台 | `Sidebar` + `Header` + `Overview/NewTask/Details/History` + `Drawer` |
...
##### A. Summary 模式 Prompt (通俗解读，零法律文本)

**设计原则**（v2.1 重构 — 解决「复读」问题）:

> **问题根因**: 与 rewrite 相同——旧 prompt 包含 `# Rules` / `Output Format` / 法律依据(legal_content) /
> violation_hint(通用合规提示)，模型在**填空模板**而非真正分析条款内容。
>
> **解决策略**:
> 1. **零法律文本** — 不出现任何法律条文或合规提示
> 2. **去掉全部格式模板** — 不再教模型"怎么排版"，只告诉他"说什么"
> 3. **违规类型 → 问题本质映射** — 告诉模型这条条款的核心问题是什么（分析锚点）
> 4. **对话式人设** — 「像跟朋友聊天一样解释」，降低正式感减少套话

**System Role**: `你是一个擅长把复杂文件翻译成普通人能听懂的话的人。你的回答应该像一个懂行的朋友在给不懂技术的朋友解释事情。简短、直接、有例子。`

**User Content 核心要素**:
```
请用大白话帮我理解下面这条隐私政策条款有什么问题。不要用任何专业术语，就像跟朋友聊天一样解释。
...
| | 旧输出（复读/填空） | 新输出（针对性分析） |
|--|---|---|
| 条款本质 | 复述原文 + 法律术语 | 用大白话重新表述实际含义 |
| 风险点拨 | 通用模板套话 | 结合具体条款内容说清运作方式和后果 |

##### B. Rewrite 模式 Prompt (编辑指令驱动，零法律文本)

**设计原则**（v2.1 重构 — 解决 Phi-4 Mini「复读」问题）:

> **问题根因**: Phi-4 Mini 等小模型有强烈的「输入复述」倾向——prompt 中出现多少法律文本，
> 模型就会在输出中复述多少。v2.0 方案即使截断到80字，模型仍照搬不误。
>
> **解决策略**:
...
**System Role**: `你是隐私政策编辑。输入有问题的条款，输出修改后的干净版本。不解释、不引用法律、不加标题。像用查找替换工具改文章一样工作。`
...
【修改要求】
{edit_instruction}   # ← 根据 violation_type 从 EDIT_INSTRUCTIONS 字典中选取
